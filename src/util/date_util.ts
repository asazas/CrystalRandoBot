...
function validate_date(date: string, timezone: string, past: boolean = false): DateTime<true> {
	const date_obj = DateTime.fromFormat(date, 'd/M/y', { zone: timezone });
	if (date_obj.isValid) {
		return date_obj;
	} else if (date === 'today') {
		...
		const yesterday = DateTime.local({ zone: timezone }).startOf('day').minus({ days: 1 });
		return yesterday;
	} else if (date in weekdays) {
		const current_weekday = DateTime.local({ zone: timezone }).weekday;
		let desired_weekday = weekdays[date];
		if (past) {
			if (desired_weekday >= current_weekday) desired_weekday -= 7;
			const previous_date = DateTime.local({ zone: timezone })
				.startOf('day')
				.minus({ days: current_weekday - desired_weekday });
			return previous_date;
		} else {
			if (desired_weekday <= current_weekday) desired_weekday += 7;
			const next_date = DateTime.local({ zone: timezone })
				.startOf('day')
 ...
 */
function validate_time(date_obj: DateTime<true>, time: string, timezone: string): DateTime<true> {
	let hours = null,
		minutes = null;
	if (time === 'NOW') {
		const now = DateTime.local({ zone: timezone }).startOf('minute');
		hours = now.hour;
		minutes = now.minute;
	...
	if (us_time) {
		...
		if (us_time[3] === 'AM' && hours === 12) hours = 0;
		else if (us_time[3] === 'PM' && hours !== 12) hours += 12;
 ...
 * @returns Luxon DateTime object representing the requested date and time.
 */
function process_date(date: string, time: string, timezone: string): DateTime<true> {
